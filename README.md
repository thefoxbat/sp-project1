...
All project requirements are fully implemented, tested, and passing.
...
bitmap_find_fisrt_bit()

Finds the position of the first bit with a specified value (0 or 1).

bitmap_set_bit()

Sets a specific bit to 1 (Allocated).

bitmap_clear_bit()

Clears a specific bit to 0 (Free).
...
Prints the contents of the bitmap (provided implementation).
...
Initializes the memory manager with a default 8-byte slot list and one batch of slots.

mem_mngr_alloc()

Allocates memory slots using the first-fit policy.

mem_mngr_free()

Frees allocated memory with comprehensive error detection.

mem_mngr_leave()

Cleans up and frees all dynamically allocated memory before program termination.
...
mingw32-make all or mingw32-make test
